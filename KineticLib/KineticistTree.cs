...
namespace KineticistElementsExpanded.KineticLib
{
    public class KineticistTree
    {
        public KineticistTree()
        {
            ...
            BloodKineticist = Helper.ToRef<BlueprintArchetypeReference>("365b50dba54efb74fa24c07e9b7a838c");
            ...
            MainStatProp = Helper.ToRef<BlueprintUnitPropertyReference>("f897845bbbc008d4f9c1c4a03e22357a");
            BlastFeature = Helper.ToRef<BlueprintFeatureReference>("93efbde2764b5504e98e6824cab3d27c");
            ...
            BaseBasicEnergy = GetAll(true, false, basicPhysical: false).Select(s => s.BaseAbility).ToArray();
            BaseBasicPhysical = GetAll(true, false, basicEnergy: false).Select(s => s.BaseAbility).ToArray();
            BaseComposite = GetAll(false, true).Select(s => s.BaseAbility).ToArray();
            BaseAll = GetAll(true, true).Select(s => s.BaseAbility).ToArray();
        }

        public IEnumerable<Element> GetAll(bool basic = false, bool composites = false, bool basicEnergy = true, bool basicPhysical = true)
        {
            if (basic)
            {
                if (basicEnergy) 
        ...
        public BlueprintArchetypeReference BloodKineticist;
        ...
        public BlueprintUnitPropertyReference MainStatProp;
        public BlueprintFeatureReference BlastFeature;

        public BlueprintAbilityReference[] BaseBasic;
        public BlueprintAbilityReference[] BaseBasicEnergy;
        public BlueprintAbilityReference[] BaseBasicPhysical;
